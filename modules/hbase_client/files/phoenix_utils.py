...
def setPath():
    ...
    # if HADOOP_CONF_DIR set explicitly, use that
    hadoop_conf_dir = os.getenv('HADOOP_CONF_DIR', os.getenv('HADOOP_CONF_PATH'))
    if not hadoop_conf_dir:
        # else fall back to HADOOP_HOME
        if os.getenv('HADOOP_HOME'):
            hadoop_conf_dir = os.path.join(os.getenv('HADOOP_HOME'), "conf")
        else:
            # default to /etc/hadoop/conf
...
def find_zookeeper():
    # Parse hbase-site to find the Zookeeper znode.
    import xml.etree.ElementTree as ET
    hbase_site = os.path.join(hbase_conf_dir, "hbase-site.xml")
    if not os.path.isfile(hbase_site):
        raise OSError("Cannot find hbase-site.xml (Tried %s)" % hbase_site)
    ...
    if "hbase.zookeeper.quorum" not in attributes or "zookeeper.znode.parent" not in attributes:
        raise Exception("Could not determine Zookeeper quorum from hbase-site.xml")
    quorum = attributes["hbase.zookeeper.quorum"] + ":" + attributes["zookeeper.znode.parent"]
    return quorum
...
if __name__ == "__main__":
    ...
    print "hadoop_conf_dir:", hbase_conf_dir
